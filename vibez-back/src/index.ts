...
// Step 1: visit /auth/spotify in the browser
...
app.get('/auth/spotify/callback', async (req, res) => {
  ...
  const playlistIdQuery = req.query.playlistId;
  const code = Array.isArray(codeQuery) ? codeQuery[0] : codeQuery;
  const spotifyError = Array.isArray(errorQuery) ? errorQuery[0] : errorQuery;
  const state = Array.isArray(stateQuery) ? stateQuery[0] : stateQuery;
  const playlistId = Array.isArray(playlistIdQuery) ? playlistIdQuery[0] : playlistIdQuery;

  if (typeof spotifyError === 'string' && spotifyError.length > 0) {
    res.status(400).json({ error: `Spotify authorization failed: ${spotifyError}` });
    return;
  }

  if (typeof code !== 'string' || code.trim().length === 0) {
    res.status(400).json({ error: 'Missing Spotify authorization code. Start authorization at /auth/spotify and complete the consent flow.' });
    return;
  }

  if (typeof state !== 'string' || state !== SpotifyService.OAUTH_STATE) {
    res.status(400).json({ error: 'Invalid OAuth state. Start a fresh flow at /auth/spotify and complete it in the same browser tab.' });
    return;
  }

    try {
        await spotify.handleCallback(code);
        const targetPlaylistId = typeof playlistId === 'string' && playlistId.trim().length > 0
          ? playlistId.trim()
          : '1hw08XyqwkURDIZobunZtA';
        const tracks = await spotify.getTracksFromPlaylist(targetPlaylistId);

        res.json({
          success: true,
          message: 'Spotify authentication succeeded and default playlist tracks were fetched.',
        ...
        const statusCode = message.includes('exchange Spotify auth code') ? 400 : 500;

        res.status(statusCode).json({
          error: message,
          hint: 'Spotify authorization codes are single-use and expire quickly. Open /auth/spotify, approve access, and use the redirected callback URL once.'
        });
    }
});
...
app.get('/spotify/playlist/:playlistId', async (req, res) => {
  ...
  if (typeof playlistId !== 'string' || playlistId.trim().length === 0) {
    res.status(400).json({ error: 'Missing playlist ID in route parameter.' });
    return;
  }

  try {
    const tracks = await spotify.getTracksFromPlaylist(playlistId.trim());
    console.log(`Fetched ${tracks.length} tracks from playlist ${playlistId.trim()}`);
    res.json({ success: true, playlistId: playlistId.trim(), trackCount: tracks.length, tracks });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    ...
    res.status(statusCode).json({
      error: message,
      hint: 'If playlist is public and this still fails, verify playlist ID and test again using a fresh /auth/spotify flow.'
    });
  }
});
